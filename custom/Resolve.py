...
MAX_NEW_TOKENS        = 128
SAFETY_GB             = 2.0  # minimum headroom inside CUDA allocator

# Expert cache caps (can grow during decode)
...
EXPERT_TOP_RATIO    = 0.50
EXPERT_VRAM_RATIO   = 0.50

# Decode-time promotion knobs
FREEPOOL_THRESHOLD_GB = 1.5
PROMOTE_EVERY_STEPS   = 4
PROMOTE_BATCH_SIZE    = 3
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    torch_dtype=DTYPE,
    device_map={"": CPU},   # fully materialized; no meta; no accelerate asserts
)
model.config.use_cache = True
model.eval()

# === Take a clean CPU snapshot for later rehydration of small modules (gates) ===
with torch.no_grad():
...
def move_trunk_except_experts_to_gpu(model: nn.Module, device: str, dtype: torch.dtype):
    dev = torch.device(device)
    for name, mod in model.named_modules():
        if "experts" in name:    # experts stay on CPU
            continue
        _to_cuda_if_has_params(mod, dev, dtype)

def rehydrate_all_gates_from_cpu_sd(model: nn.Module, cpu_sd: dict, device: str):
    """
    Force-load gate (and gate.proj if present) weights from the pristine CPU state_dict.
    This guarantees non-empty, correct-sized weights live on GPU before prefill.
    """
    dev = torch.device(device)
    fixed = 0
    with torch.no_grad():
        for li, layer in enumerate(getattr(model, "model").layers):
            if hasattr(layer, "mlp") and hasattr(layer.mlp, "gate"):
                g = layer.mlp.gate
                g.to(dev, dtype=DTYPE)
                # Rehydrate direct gate params
                prefix = None
                # Find module's prefix by scanning one known param name
                for n, p in g.named_parameters(recurse=True):
                    # full model key ends with this local name; find prefix
                    # We search for exact match across state_dict
                    for full_k in cpu_sd.keys():
                        if full_k.endswith(f".layers.{li}.mlp.gate.{n}"):
                            prefix = full_k[:-len(n)]
                            break
                    break
                # Fallback: generic search if prefix not found
                if prefix is None:
                    # Try all keys that contain .layers.{li}.mlp.gate.
                    candidates = [k for k in cpu_sd.keys() if f".layers.{li}.mlp.gate." in k]
                    for k in candidates:
                        local = k.split(f".layers.{li}.mlp.gate.",1)[1]
                        tgt = g
                        parts = local.split(".")
                        for c in parts[:-1]:
                            if hasattr(tgt, c): tgt = getattr(tgt, c)
                        leaf = parts[-1]
                        if hasattr(tgt, leaf):
                            getattr(tgt, leaf).data = cpu_sd[k].to(dev, dtype=DTYPE)
                            fixed += 1
                else:
                    # load by walking parameters/buffers
                    for n, _ in g.named_parameters(recurse=True):
                        full = prefix + n
                        if full in cpu_sd:
                            getattr(g, n.replace(".", ".")).data = cpu_sd[full].to(dev, dtype=DTYPE)  # safe
                            fixed += 1
                    for n, _ in g.named_buffers(recurse=True):
                        full = prefix + n
                        if full in cpu_sd:
                            getattr(g, n.replace(".", ".")).data = cpu_sd[full].to(dev, dtype=DTYPE)
                            fixed += 1

                # Ensure optional gate.proj is moved + loaded
                if hasattr(g, "proj"):
                    g.proj.to(dev, dtype=DTYPE)
                    # brute-force assign from SD for proj.* if present
                    for n, _ in g.proj.named_parameters(recurse=True):
                        key = f"model.layers.{li}.mlp.gate.proj.{n}"
                        if key in cpu_sd:
                            getattr(g.proj, n).data = cpu_sd[key].to(dev, dtype=DTYPE); fixed += 1
                    for n, _ in g.proj.named_buffers(recurse=True):
                        key = f"model.layers.{li}.mlp.gate.proj.{n}"
                        if key in cpu_sd:
                            getattr(g.proj, n).data = cpu_sd[key].to(dev, dtype=DTYPE); fixed += 1

    # Sanity: verify non-empty, right device
    for li, layer in enumerate(getattr(model, "model").layers):
        if hasattr(layer, "mlp") and hasattr(layer.mlp, "gate"):
            for p in layer.mlp.gate.parameters():
                assert p.numel() > 0, f"Gate param empty at layer {li}"
                assert p.device.type == torch.device(device).type, f"Gate param on {p.device}, expected {device}"
            if hasattr(layer.mlp.gate, "proj"):
                for p in layer.mlp.gate.proj.parameters():
                    assert p.numel() > 0, f"Gate.proj param empty at layer {li}"
                    assert p.device.type == torch.device(device).type, f"Gate.proj param on {p.device}, expected {device}"
    print(f"[Gates] rehydrated/moved params: {fixed}")
...
print("Moving trunk (except experts) to GPU …")
move_trunk_except_experts_to_gpu(model, GPU, DTYPE)

print("Rehydrating all gates from CPU snapshot …")
rehydrate_all_gates_from_cpu_sd(model, cpu_sd, GPU)
...
# ================= Rank experts by prefill usage & prefetch =============
...
def maybe_bump_caps():
    ...
    if target > expert_cache.max_bytes:
        expert_cache.max_bytes = target  # bounded by headroom; LRU evicts if needed
