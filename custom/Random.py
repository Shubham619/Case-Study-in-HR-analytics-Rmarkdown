...
class ExpertGPUCache:
    ...
    # FIX: Ensure the GPU module is created without copying weights from CPU
    # The original deepcopy path in the `else` block was flawed.
    def _build_cuda_module(self, cpu_expert: nn.Module) -> nn.Module:
        cls = cpu_expert.__class__
        
        # 1. Preferred: Re-instantiate using config if available (clean structure)
        if hasattr(cpu_expert, "config"):
            m = cls(config=cpu_expert.config).to(self.device, dtype=self.dtype)
        else:
            # 2. Fallback: Deepcopy structure, zero out weights, then move to GPU.
            # This avoids copying the CPU weights and then moving them to GPU.
            m_cpu_copy = copy.deepcopy(cpu_expert).cpu() 
            # Zero out weights and buffers before moving to GPU
            with torch.no_grad():
                for p in m_cpu_copy.parameters(): p.data.zero_()
                for b in m_cpu_copy.buffers(): b.data.zero_()
            m = m_cpu_copy.to(self.device, dtype=self.dtype)
    ...
    def _load_weights_async(self, cuda_mod: nn.Module, expert_id:int):
        ...
        with torch.cuda.stream(self.prefetch_stream):
            for k, v_cpu in sd.items():
                ...
                if hasattr(ref, leaf):
                    t = getattr(ref, leaf)
                    # Check if the tensor size matches before copying
                    if t.data.shape == v_cpu.shape:
                        t.data.copy_(v_cpu, non_blocking=True)
                    else:
                        print(f"Warning: Expert {expert_id} parameter {k} size mismatch. Skipping copy.")
...
def make_cached_forward(expert_id:int, cpu_expert: nn.Module):
    def wrapped_forward(*args, **kwargs):
        ...
        def to_cuda(x): 
            # Ensure only tensors on the CPU are moved. Tensors already on GPU stay.
            if torch.is_tensor(x) and x.device.type == 'cpu': 
                return x.to(cuda_dev, non_blocking=True)
            return x
            
        # The expert input (from router/gate) should be on GPU, but this handles robustness.
...
def probe_expert_bytes(eid_probe:int) -> int:
    ...
    # Use the corrected _build_cuda_module logic to get the size
    if hasattr(cpu_mod, "config"):
        tmp = cls(config=cpu_mod.config).to(gpu_device, dtype=dtype)
    else:
        # Fallback path, creating structure, zeroing weights
        tmp_cpu_copy = copy.deepcopy(cpu_mod).cpu()
        with torch.no_grad():
            for p in tmp_cpu_copy.parameters(): p.data.zero_()
            for b in tmp_cpu_copy.buffers(): b.data.zero_()
        tmp = tmp_cpu_copy.to(gpu_device, dtype=dtype)
...
def maybe_bump_caps():
    ...
    # Only bump if it's more than the current cap and less than the overall global max
...
def promote_some_experts():
    ...
    while promotion_queue and len(batch) < PROMOTE_BATCH_SIZE:
        ...
        # Check if expert is already in cache (shouldn't be, but for safety)
        if eid_ not in expert_cache._cache and eid_ in cpu_expert_registry:
            batch.append(eid_)
    if batch:
...
for step in range(max_new):
    with torch.inference_mode():
        ...
        # Update the mask for the token that was just generated (at index cur_len)
        attn_mask[:, cur_len] = 1
        cur_len += 1
